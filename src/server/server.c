...
void parseMessage (int sendingTeam, const unsigned char *buf, int nbbytes) {
  ...
  switch (buf[4]) {
    ...
    case MSG_SCORE:
    {
      ...
      if (nbbytes < 6) {
	log (KRED, "*** SCORE message is too short (%d bytes) ***\n", nbbytes);
	return;
      }
      ...
      if (buf[3] != 0xFF) {
	log (KRED, "*** SCORE messages not sent to server are ignored ***\n");
	break;
      }
      /*
	...
	break;
      ...
      scoreValue = *((uint8_t *) &buf[5]);

      if ((scoreValue != 1) && (scoreValue != 3)) {
	log (KRED, "*** SCORE can be equal to 1 or 3 only ***\n");
      }

      log (COL(buf[3]-1), "%s\n", game.teams[buf[3]-1].name);
      log (KNRM, alinea);
      log (KNRM, "          SCORE      score=%d\n", scoreValue);

      game.teams[buf[3]-1].score += scoreValue;
...
int main (int argc, char **argv) {
  ...
  for (team=0; team<15; team++){
	...
	printf("Score of %s is %d\n", game.teams[i].name,  game.teams[i].score);
      }
    }
